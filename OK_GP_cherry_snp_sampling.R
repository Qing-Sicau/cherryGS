...
# =====================================================================================
# Part 1: Find Python Path Before Parallelization (working one)
# =====================================================================================
...
for (TRAIT_OF_INTEREST in ALL_TRAITS_IN_FILE) {
  ...
  #  Cluster setup is now INSIDE the trait loop
  log_message("SETUP: Setting up robust parallel backend to use ", NUM_CORES, " cores for trait: ", TRAIT_OF_INTEREST)
  ...
  parallel::clusterEvalQ(cl, {
    ...
    Sys.setenv(TRAINING_EPOCHS = 100)
  ...
  log_message("SETUP: Parallel cluster is ready for the current trait.")
  ...
  # Hyper parameter tunning once, and used throughout
  log_message("PREP: Performing one-time DL hyperparameter tuning on FULL dataset for trait: ", TRAIT_OF_INTEREST)
  set.seed(123)
  # using the complete matrix (genotypeMatrix_full) 
  tuning_indices <- createDataPartition(phenotypeVector, p = 0.8, list = FALSE)
  tuning_genotype <- genotypeMatrix_full[tuning_indices, ]
  validation_genotype <- genotypeMatrix_full[-tuning_indices, ]
  ...
  tryCatch({
    ...
    for(j in 1:nrow(mlp_param_grid)) {
        params <- mlp_param_grid[j,]; model <- keras_model_sequential() %>%
          layer_dense(units=params$neurons, input_shape=numMarkers_full, kernel_regularizer = regularizer_l2(l2 = 0.001)) %>%
  ...
  }, error=function(e){log_message("WARNING: MLP tuning failed. Using defaults.", indent=1)})
  log_message("PREP: Best MLP params found: neurons=", best_mlp_params$neurons, ", dropout=", best_mlp_params$dropout_rate, ", lr=", best_mlp_params$learning_rate, indent=1)

  # --- CNN tunning ---
  best_cnn_params <- list(filters=32, kernel_size=8, learning_rate=0.005)
  tryCatch({
    xtrain_cnn_tune <- array(tuning_genotype_scaled, dim=c(nrow(tuning_genotype_scaled), numMarkers_full, 1))
    xval_cnn_tune <- array(validation_genotype_scaled, dim=c(nrow(validation_genotype_scaled), numMarkers_full, 1))
    ...
    for(j in 1:nrow(cnn_param_grid)) {
        params <- cnn_param_grid[j,]; model <- keras_model_sequential() %>%
          layer_conv_1d(filters=params$filters, kernel_size=params$kernel_size, input_shape=c(numMarkers_full,1), kernel_regularizer = regularizer_l2(l2 = 0.001)) %>%
  ...
  }, error=function(e){log_message("WARNING: CNN tuning failed. Using defaults.", indent=1)})
  log_message("PREP: Best CNN params found: filters=", best_cnn_params$filters, ", kernel_size=", best_cnn_params$kernel_size, ", lr=", best_cnn_params$learning_rate, indent=1)
  ...
  for (density in SNP_DENSITIES) {
    ...
    run_one_repetition <- function(rep_id, tuned_mlp_params, tuned_cnn_params) {
      ...
      for (i in 1:length(folds)) {
        ...
        pheno_with_NAs <- phenotypeVector
        pheno_with_NAs[test_indices] <- NA
  ...
  log_message("MEMORY: Explicitly cleaning up large objects before processing the next trait...")
    
  # clean up large matrix in the main process
  rm(results_df, all_results_list, summary_stats, plot_density_decay)
  
  if (exists("G")) rm(G)
  if (exists("D")) rm(D)
  if (exists("H")) rm(H)
  if (exists("genotypeMatrix")) rm(genotypeMatrix)
  if (exists("genotypeMatrix_sub")) rm(genotypeMatrix_sub)
  
  # force memory clean and relocation in the main process (防止爆内存！！)
  gc(full = TRUE, verbose = FALSE)
  log_message("MEMORY: Cleanup complete for the main process.", indent = 1)

  # Cluster shutdown is now INSIDE the trait loop
  log_message("CLEANUP: Stopping the parallel cluster to release all worker memory...")
  parallel::stopCluster(cl)
  plan(sequential) # Reset plan to sequential to be safe
  log_message("CLEANUP: Cluster stopped. Memory fully released. Ready for the next trait.", indent=1)


} # <<< End of the main "for" loop for traits
