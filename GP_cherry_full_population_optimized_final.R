# =====================================================================================
#
#   Script 1: Full Population Genomic Prediction (V2 - Automated Multi-Trait)
#
# Description:
#   This script evaluates multiple genomic prediction models on the full population.
#   It has been upgraded to automatically iterate through all traits found in the
#   phenotype file, performing a complete analysis for each one. Outputs are
#   systematically named based on the trait being analyzed.
#
...
cat(paste("[DATA] Data preparation complete: ", numIndividuals, " individuals, ", numMarkers, " SNP markers.\n"))


# =====================================================================================
# Part 4: Pre-calculate Relationship Matrices and Pedigree
# =====================================================================================
# These matrices are calculated once and reused for each trait analysis.
...
cat("[PREP] A-matrix (pedigree) built successfully.\n")

hMatrixInverse <- doH_1_inverse(A_full, G)
cat("[PREP] H-inverse matrix (for ssGBLUP) built successfully.\n")
...
for (TRAIT_OF_INTEREST in ALL_TRAITS_IN_FILE) {
  ...
  cat(paste0("\n\n######################################################################\n"))
  cat(paste0("###   STARTING ANALYSIS FOR TRAIT: ", TRAIT_OF_INTEREST, "   ###\n"))
  cat(paste0("######################################################################\n"))
  ...
  run_one_repetition <- function(rep_id) {
    ...
    cat(paste0("  [CV] Trait: ", TRAIT_OF_INTEREST, " - Repetition ", rep_id, "/", NUM_REPEATS, "...\n"))
    set.seed(42 + rep_id)
    ...
    for (i in 1:length(folds)) {
      ...
      tryCatch({
          ...
          best_val_loss <- Inf
          best_mlp_params <- list(neurons = NA, dropout_rate = NA, learning_rate = NA)
          for(j in 1:nrow(mlp_param_grid)) {
              params <- mlp_param_grid[j, ]
              model <- keras_model_sequential() %>%
                  layer_dense(units=params$neurons, input_shape=numMarkers, kernel_regularizer=regularizer_l2(l=0.001)) %>%
                  layer_batch_normalization() %>% layer_activation_relu() %>% layer_dropout(rate=params$dropout_rate) %>%
                  layer_dense(units=round(params$neurons/2), kernel_regularizer=regularizer_l2(l=0.001)) %>%
              ...
              hist <- model %>% fit(genotypeTrain_scaled, phenotypeTrain, epochs=TRAINING_EPOCHS, batch_size=32, validation_split=0.2, verbose=0, callbacks=callbacks_list)
          ...
          final_model <- keras_model_sequential() %>%
              layer_dense(units=best_mlp_params$neurons, input_shape=numMarkers, kernel_regularizer=regularizer_l2(l=0.001)) %>%
              layer_batch_normalization() %>% layer_activation_relu() %>% layer_dropout(rate=best_mlp_params$dropout_rate) %>%
              layer_dense(units=round(best_mlp_params$neurons/2), kernel_regularizer=regularizer_l2(l=0.001)) %>%
              layer_batch_normalization() %>% layer_activation_relu() %>% layer_dense(units=1)
          final_model %>% compile(loss="mse", optimizer=optimizer_adam(learning_rate=best_mlp_params$learning_rate))
          final_callbacks <- list(callback_early_stopping(monitor="loss", patience=10))
      ...
      tryCatch({
          ...
          best_val_loss <- Inf
          best_cnn_params <- list(filters=NA, kernel_size=NA, learning_rate=NA)
          for(j in 1:nrow(cnn_param_grid)) {
              params <- cnn_param_grid[j, ]
              model <- keras_model_sequential() %>%
                  layer_conv_1d(filters=params$filters, kernel_size=params$kernel_size, input_shape=c(numMarkers,1), kernel_regularizer=regularizer_l2(l=0.001)) %>%
              ...
              hist <- model %>% fit(xtrain_cnn, phenotypeTrain, epochs=TRAINING_EPOCHS, batch_size=32, validation_split=0.2, verbose=0, callbacks=callbacks_list)
              val_loss <- min(hist$metrics$val_loss, na.rm=TRUE)
              if (is.finite(val_loss) && val_loss < best_val_loss) { best_val_loss <- val_loss; best_cnn_params <- params }
          }
          final_model <- keras_model_sequential() %>%
              layer_conv_1d(filters=best_cnn_params$filters, kernel_size=best_cnn_params$kernel_size, input_shape=c(numMarkers,1), kernel_regularizer=regularizer_l2(l=0.001)) %>%
              layer_batch_normalization() %>% layer_activation_relu() %>% layer_max_pooling_1d(pool_size=4) %>% layer_flatten() %>%
              layer_dense(units=64, kernel_regularizer=regularizer_l2(l=0.001)) %>%
              layer_batch_normalization() %>% layer_activation_relu() %>% layer_dense(units=1)
  ...
  results_df <- future_map_dfr(.x = 1:NUM_REPEATS, .f = run_one_repetition, .options = furrr_options(seed = TRUE))
  ...
  # --- Save Results and Plots (UPGRADED NAMING) ---
  ggsave(paste0("plots/GS_Model_Comparison_", TRAIT_OF_INTEREST, ".pdf"), plot = plot_main, width = 10, height = 8, device = "cairo_pdf")
  write.csv(results_df, paste0("results/GS_All_Results_", TRAIT_OF_INTEREST, ".csv"), row.names=FALSE)
  write.csv(summary_stats, paste0("results/GS_Summary_", TRAIT_OF_INTEREST, ".csv"), row.names = FALSE)
  # UPGRADE: Save Tukey HSD results to a CSV file
